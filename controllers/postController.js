...
exports.getProfilePage = async (req, res, next) => {
  try {
    ...
    userPage.imageUpload[0].image = `${
      process.env.API_URL + "/uploads/profile/"
    }${userPage.imageUpload[0].image}`;
    const numberPosts = await post.find({ userId: userId }).count();
    const posts = await post
      .find({ userId: userId })
      .select("title caption")
      .populate({ path: "imageId", select: "image" })
      .sort({ createdAt: "desc" });
    for (const item of posts) {
      item.imageId.image = `${API_URL + "/uploads/post/"}${item.imageId.image}`;
    }
    ...
    res
      .status(200)
      .json({ userPage, numberPosts, posts, numberFollower, numberFollowing });
  } catch (error) {
    next(error);
  }
};
...
exports.getMainPage = async (req, res, next) => {
  try {
    ...
    const following = await follow
      .find({ FalowerId: req.userId })
      .select("-_id FallowingId");
    const followingId = [];
    ...
    for (const item of posts) {
      item.imageId.image = `${API_URL + "/uploads/post/"}${item.imageId.image}`;
      const text = item.userId.imageUpload[0].image.search(
        process.env.API_URL + "/uploads/profile/"
      );
      if (text === -1) {
        item.userId.imageUpload[0].image = `${
          process.env.API_URL + "/uploads/profile/"
        }${item.userId.imageUpload[0].image}`;
      }
    }
...
exports.getSinglePost = async (req, res, next) => {
  try {
    ...
    postUser.imageId.image = `${API_URL + "/uploads/post/"}${
      postUser.imageId.image
    }`;
    postUser.userId.imageUpload[0].image = `${
      process.env.API_URL + "/uploads/profile/"
    }${postUser.userId.imageUpload[0].image}`;
    const commentPost = await comment
      .find({ postId: postUser._id })
      .select("_id text")
      .populate({
        path: "userId",
        select: "fullName userName imageUpload",
        populate: {
          path: "imageUpload",
          match: { type: "profile" },
          select: "image",
