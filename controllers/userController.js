  ...
  try {
    ...
    res
      ...
      .json({
        ...
        filePath: `${process.env.API_URL}/uploads/post/${fileName}`,
        imageId: img._id,
      });
  } catch (err) {
    next(err);
...
exports.createProfile = async (req, res, next) => {
  try {
    ...
    const oldProfile = await imageUpload.findOne({
      userId: req.userId,
      type: "profile",
    });
    if (oldProfile) {
      await fs.unlinkSync(
        `${appRoot}/public/uploads/profile/${oldProfile.image}`
      );
    ...
    UserAddImage.imageUpload.push(img._id);
    UserAddImage.save();
    res.status(200).json({ message: "با موفقیت اضافه شد" });
  } catch (error) {
    next(error);
  }
};
...
exports.getAllFollowing = async (req, res, next) => {
  try {
    ...
    const followings = await follow
      .find({ FalowerId: userId })
      .populate("FallowingId");
...
exports.getEdit = async (req, res, next) => {
  try {
    const userEdit = await User.findById(req.userId).select(
      "-_id fullName email userName phone"
    );
    if (!userEdit) {
